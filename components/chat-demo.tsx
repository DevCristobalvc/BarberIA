...
export function ChatDemo() {
  ...
  return (
    <div className="w-full max-w-xs mx-auto rounded-3xl overflow-hidden shadow-2xl border border-[#2A2A2A]" style={{ background: "#0A0A0A" }}>
      {/* Header */}
      <div className="px-4 py-3 flex items-center gap-3 border-b border-[#1A1A1A]" style={{ background: "#111111" }}>
        <div className="w-9 h-9 rounded-full bg-gold flex items-center justify-center text-[#0A0A0A] text-sm font-bold flex-shrink-0">S</div>
        <div>
        ...
        {MSGS.map((msg) => (
          <div
            ...
            style={{
              ...
              transform: visible.includes(msg.id) ? "translateY(0)" : "translateY(6px)",
